...
- Refactored the AI item pipeline around shared frontend AI action hooks plus a leaner backend cleanup: image-source policy now lives in the models/helpers that own it, while the existing clean-image, transparent-PNG, and metadata generator services remain the main execution path instead of being wrapped in a second backend abstraction layer.
...
- Reworked AI image cleanup into a two-variant pipeline: `AI clean image` now generates a hidden high-contrast working image for reliable cutouts, derives a white-background display image for the UI, and makes transparent-PNG generation run from the hidden working attachment instead of the visible cleaned preview.
- Removed the hidden transparent-PNG pass from `AI clean image` itself, so the clean step now only recolors the generated backdrop for white display preview and never runs cutout logic unless the user explicitly presses `Make transparent PNG`.
- Made AI clean backdrop selection inspect the uploaded image pixels directly instead of relying mainly on sparse metadata, so blue garments can choose a truly contrasting backdrop like yellow/orange instead of falling back to a similar blue.
- Shrunk the working clean-image backdrop palette down to a tighter set of high-separation safety colors so the model is less likely to choose a backdrop that sits too close to the garment hue.
- Fixed saved-item transparent PNG generation so the backend can open `cleaned_photo` Active Storage attachments correctly instead of trying to call `read` on the attachment wrapper.
...
- Updated the transparent-PNG background remover to sample and strip the actual corner backdrop color from AI-cleaned images instead of only treating near-white edges as removable background.
- Tightened the AI clean-image prompt to demand a single uniform high-contrast backdrop with visible edge margin, and made the transparent remover choose the dominant corner backdrop color for more reliable cutouts.
- Changed the transparent-PNG flow to return the actual generated cutout result instead of silently falling back to the safer cleaned image, so users can see the transparent version directly and undo it if needed.
- Tightened the AI clean-image prompt to explicitly forbid cast, floor, contact, and drop shadows behind garments so the follow-up transparent PNG step is less likely to keep a shadow silhouette.
- Split AI image cleanup into two explicit steps: `AI clean image` now saves or stages a non-transparent catalog-style image first, and `Make transparent PNG` runs the background-removal pass separately for both saved items and create-time previews.
...
- Added persistent existing-item Undo in the metadata header and moved the existing item editor to autosave, so metadata edits, image replacements, AI clean saves, and transparent-PNG saves can be reversed one persisted step at a time without a manual save button.
- Added a visible Undo button to the unsaved manual create-item flow so upload, AI clean, transparent-PNG, and AI autofill changes can be reversed before the item exists.
- Added clean-image variant tracking plus transparent-cutout fallback metadata so the app can preserve the safer non-transparent cleaned image when a white-on-white cutout looks unreliable.
