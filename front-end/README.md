...
The current frontend supports authenticated closet management, item creation and editing, item visual descriptions, private modeled-preview consent, AI-assisted metadata, image cleanup, outfit generation flows, saved outfits and magazines, reusable private PNG decorations, admin-only user pages, and outfit-photo import that can be reviewed and converted into closet items.
...
- `/decorations` private reusable PNG decoration library and image editor
- `/magazines/new` full-page magazine creation workspace
- `/magazines/:id/edit` full-page visual magazine designer with metadata, outfit selection/order, modeled/flat-lay controls, editable text, and draggable page layers
- `/magazines/:id` clean fullscreen magazine reader, normally launched in a separate tab, with its Edit action revealed from the top edge
...
- `/outfits` includes separate `All outfits` and `My Magazines` views. The magazine library lets users create a magazine, add or reorder saved outfits, open the finished magazine, or delete the magazine without deleting its source outfits. Creating or editing navigates to a dedicated full-page workspace whose scrollable outfit picker displays each modeled preview when available, falls back to the saved flat lay, and supports fuzzy search across outfit and contained-piece metadata. Its icon-only AI action uses an optional draft title/notes as a creative brief, then fills an editable concept and selects and orders matching saved outfits without persisting until the user saves. Existing magazines also expose a visual page designer: users choose modeled or flat-lay imagery per outfit page, drag imagery/title/body layers, edit page copy, and place, resize, rotate, or remove reusable decorations. Saving persists those percentage-based page layouts. Opening `/magazines/:id` in a separate tab renders the same pages in a clean app-shell-free fullscreen reader with animated page turns and arrow-key controls; only hovering or focusing the top edge reveals the Edit action. Legacy magazine-only uploads remain readable.
- `/decorations` is the reusable source-of-truth for private PNG assets. Bulk selection uploads each PNG independently and adds successful files to the grid as soon as they finish. Every asset can be renamed, deleted, or opened in the same full-screen crop, rotate, magic-wand, undo/redo editor used by item and modeled-preview imagery. In the saved-outfit editor, decorations appear beside clothing pieces in the layers rail and use the same direct drag, corner-resize, and rotation handles on the flat lay. Their placement persists separately from garment collage geometry, while saved cards, magazine flat-lay fallbacks, and modeled-outfit snapshot references all render the same visible PNG bounds.
- The saved outfits collection is cached in `App.tsx` after the first `/outfits` load. Navigating away and back reuses that cache, while outfit creation, edits, deletes, AI generation, and clothing-item edits patch the cached records directly. Outfit search uses the same shared suggestion-field interaction as closet search, matching fuzzy metadata across the outfit and its pieces, showing matching-piece context and a live result count, and opening the highlighted outfit with Enter. `components/outfits/useOutfitMagazines.ts` owns the gallery's magazine collection and deletion state; the route-level editor and reader independently load the data they need so direct URLs and separate tabs work reliably.
- The saved-outfit collage editor is library-backed: `react-moveable` owns the shared move/resize/rotate controls for clothing pieces and decorations, while `OutfitCollageCanvas` keeps measured visible image bounds and persisted item/decoration placement data in sync.
...
- Image-based item creation accepts multiple photos in one selection. Before detection, every source appears in the top strip; selecting one focuses that photo so it can be replaced, deleted, cropped, or background-erased without removing the rest of the batch. Detection then submits each edited source independently to `POST /outfit_uploads` with bounded concurrency. Detections append to the same review strip as each photo finishes, while crop previews and image-editor actions continue to use the correct source photo for every detected item. Users can review and save early results without waiting for the remaining photos.
...
- `src/app/components/MagazineEditorPage.tsx`, `src/app/components/MagazineEditorForm.tsx`, and `src/app/components/magazines/`
  Full-page magazine loading/saving, metadata, searchable outfit selection, ordering, AI concept autofill, and a shared visual page canvas for modeled/flat-lay imagery, text, and decoration layout
- `src/app/components/MagazineReaderPage.tsx`
  Clean fullscreen animated reader that renders saved page layouts and reveals its return-to-editor action from the top edge
...
- `src/app/components/OutfitCollageCanvas.tsx`
  Shared saved-outfit renderer and `react-moveable` interaction layer for clothing pieces and visible-bounds-cropped decorations, using the same normalized stage contract in gallery and editor views
- `src/app/components/OutfitCollageLayersPanel.tsx`
  Clothing-and-decoration thumbnail rail with selection, add/remove controls, and pointer/keyboard-accessible clothing layer reordering
