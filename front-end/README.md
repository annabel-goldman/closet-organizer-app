...
- `/decorations` is the reusable source-of-truth for private PNG assets. Bulk selection uploads each PNG independently and adds successful files to the grid as soon as they finish. Every asset can be renamed, deleted, or opened in the same full-screen crop, rotate, magic-wand, undo/redo editor used by item and modeled-preview imagery. The saved-outfit flat-lay editor exposes its own decoration plus control and persists placement separately from garment collage geometry; saved cards, magazine flat-lay fallbacks, and modeled-outfit snapshot references all render those placements.
